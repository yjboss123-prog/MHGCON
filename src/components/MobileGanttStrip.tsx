...
interface MobileGanttStripProps {
  tasks: Task[];
  language: 'en' | 'fr';
  userToken?: string;
...
export function MobileGanttStrip({ tasks, language, userToken, isContractor }: MobileGanttStripProps) {
  ...
  const TOTAL_WIDTH = TASK_NAME_WIDTH + (48 * WEEK_WIDTH);
    ...
    >
      <div style={{ width: `${TOTAL_WIDTH}px`, minWidth: `${TOTAL_WIDTH}px` }}>
        <div className="sticky top-0 bg-white z-10 border-b-2 border-slate-300">
          <div className="flex">
            <div
              className="border-r border-slate-200 p-2 font-semibold text-sm text-slate-700"
            ...
            <div className="flex">
              {Array.from({ length: 12 }).map((_, monthIdx) => (
                <div
                  ...
                  style={{ width: `${4 * WEEK_WIDTH}px`, minWidth: `${4 * WEEK_WIDTH}px` }}
                >
                  {monthNames[monthIdx]}
                </div>
              ))}
        ...
        <div>
          {tasks.map((task, idx) => {
            ...
            return (
              ...
              >
                <div
                  className="border-r border-slate-200 p-2 text-sm text-slate-700 truncate"
                  style={{ width: `${TASK_NAME_WIDTH}px`, minWidth: `${TASK_NAME_WIDTH}px` }}
                >
                  {task.name}
                </div>
                <div
                  ...
                  style={{ width: `${48 * WEEK_WIDTH}px`, minWidth: `${48 * WEEK_WIDTH}px` }}
                >
                  ...
                  <div
                    ...
                    style={{
                      left: `${(left / 48) * (48 * WEEK_WIDTH)}px`,
                      width: `${(width / 48) * (48 * WEEK_WIDTH)}px`,
                    }}
                  />
                </div>
              </div>
